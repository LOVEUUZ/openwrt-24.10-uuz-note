...
return view.extend({
	...
	render: function(lanIP) {
	    ...
	    // Flag 开关示例
	    o = s.option(form.Flag, "enabled", "启用功能");
	    o.default = 1;                     // 默认勾选
	    o.rmempty = false;                 // 强制必须勾选或不勾选

	    // ListValue 下拉选择框示例
	    o = s.option(form.ListValue, "protocol", "协议类型");
	    o.value("dhcp", "DHCP");           // value 对应存储值，第二个是显示文字
	    o.value("static", "静态IP");
	    o.value("pppoe", "PPPoE");
	    o.default = "pppoe";                // 默认选项

	    // DynamicList 动态列表示例
	    o = s.option(form.DynamicList, "dns_servers", "DNS 服务器列表");
	    o.datatype = "host";               // 校验每一行是合法主机名或 IP
	    o.placeholder = "8.8.8.8";         // 提示输入示例

	    // TextValue 多行文本示例
	    o = s.option(form.TextValue, "custom_script", "自定义脚本");
	    o.rows = 10;                        // 文本域高度（行数）
	    o.wrap = "off";                     // 不换行

	    // Button 按钮示例
	    o = s.option(form.Button, "apply_btn", "应用设置");
	    o.inputtitle = "立即应用";           // 按钮文字
	    o.inputstyle = "apply";             // 样式，可选 apply/reset/delete
	    o.onclick  = function() {
					console.log("应用按钮被点击了");
	    };
